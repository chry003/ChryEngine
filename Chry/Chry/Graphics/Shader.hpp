...
namespace Chry {
	...
	struct BufferElement
	{
		...
		BufferElement(ShaderDataType type, const sstr& name, bool normalized = false)
	...
	{
	public:
		Shader(const sstr& name, const sstr& vertexSrc, const sstr& fragmentSrc);
		~Shader();

		void Bind() const;
		void Unbind() const;

		void SetInt(const sstr& name, int value);
		void SetIntArray(const sstr& name, int* values, int count);
		void SetFloat(const sstr& name, float value);
		void SetFloat2(const sstr& name, const glm::vec2& value);
		void SetFloat3(const sstr& name, const glm::vec3& value);
		void SetFloat4(const sstr& name, const glm::vec4& value);
